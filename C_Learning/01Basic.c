...
void time_gap() {
	...
	if (hour1 < hour2) {
		if (minute1 < minute2) {
			printf("两地时间相差%d时%d分", hour2 - hour1, minute2 - minute1);
		}
		else {
			printf("两地时间相差%d时%d分", hour2 - hour1 - 1, minute2 + 60 - minute1);
		}
	}
	else {
		if (minute1 < minute2) {
			printf("两地时间相差%d时%d分", hour1 - hour2 - 1, minute1 + 60 - minute2);
		}
		else {
			printf("两地时间相差%d时%d分", hour1 - hour2, minute1 - minute2);
		}
	}
}

void time_gap2() { // 判断时间差
	int hour1, minute1;
	int hour2, minute2;
	printf("请输入第1个时间\n");
	scanf_s("%d %d", &hour1, &minute1);
	...
	if (gap > 0) {
		printf("两地时间相差%d时%d分", gap / 60, gap % 60);
	}
	else {
		printf("两地时间相差%d时%d分", -gap / 60, -gap % 60);
	}
...
void how_to_gather_100() { // 用1,2,5元的组合去获取100元，列举出所有结果。
...
void how_to_gather_100_2() { // 用1,2,5元的组合去获取100元，一旦获取到一个结果，就跳出循环。
	int exit = 0;
	for (int one = 0; one <= 100; ++one) {
		for (int two = 0; two <= 50; ++two) {
			for (int five = 0; five <= 20; ++five) {
...
void how_to_gather_100_3() { // 用1,2,5元的组合去获取100元，一旦获取到一个结果，就跳出循环。
	int exit = 0;
	for (int one = 0; one <= 100; ++one) {
		for (int two = 0; two <= 50; ++two) {
			for (int five = 0; five <= 20; ++five) {
